 ...
 // const colorScale = d3.scale.ordinal().range(colors.seedColors);
  ...
  //  if (status.params || status.data) {
      ...
      this._updateData(data);
	  	var words=this._tagCloud._words;
												...
												/*	if (data.columns.length === 3) {													
													console.dir("data.columns.length");
													console.dir(this._tagCloud._svgGroup.selectAll('text')[0].length);
													console.dir(data.columns.length);
	...
	console.dir("value");
														...
														/*	if(value.colorSchemaValue!=undefined)
															{
																value.tagColor=itemcolors[value.colorSchemaValue];
																val.style.fill=getFill(value);
															}else{
																value.tagColor="rgb(87, 193, 123)";
																val.style.fill=getFill(value);
															}*/
