...
function TableOptions({
  ...
  $(document).ready(function () {
    $("button[data-test-subj='visualizeEditorRenderButton']").click(
      function () {
    ...
    for (var obj of aggs.indexPattern.fields) {
      if (
        labels != undefined &&
        JSON.parse(labels)[obj.name] != undefined &&
        JSON.parse(labels)[obj.name].length > 0
      ) {
        fieldsList.push({
          name: obj.name,
          xname: JSON.parse(labels)[obj.name],
        });
      } else {
   ...
   /* $("#btnViewUp1").click(function (e) {
    ...
    $("#btnViewDown1").click(function (e) {
      ...
      $("#btnViewUp1").unbind("click").click(function (e) {
      moveUp("#viewList");
    });

    $("#btnViewDown1").unbind("click").click(function (e) {
      moveDown("#viewList");
    });
   ...
   /* $("#btnExportUp1").click(function (e) {
	...
	$("#btnExportUp1").unbind("click").click(function (e) {
      moveUp("#exportList");
    });

    $("#btnExportDown1").unbind("click").click(function (e) {
      moveDown("#exportList");
    });
    ...
    function moveUp(list) {
      $("button[data-test-subj='visualizeEditorRenderButton']").removeAttr(
        "disabled"
      );
    ...
    for (var i = 0; i < fieldsList.length; i++) {
      if (
        fieldsList[i].xname !== "_source" &&
        fieldsList[i].xname !== "_id" &&
        fieldsList[i].xname !== "_index" &&
        fieldsList[i].xname !== "_score" &&
        fieldsList[i].xname !== "_type"
      ) {
        arr = fieldsList[i];
        optionsAsString +=
          "<option value='" +
    ...
    $('select[id="fieldsList"]').find("option").remove();
    ...
    <EuiPanel paddingSize="s" id="fieldsList1">
      ...
      <div className="form-group">
        <a ng-click="jsonEnable()" id="jsonId">
          <i
            aria-hidden="true"
            className="fa fa-caret-down fa-caret-left"
            ng-class="{'fa-caret-down': advancedToggled, 'fa-caret-left': !advancedToggled}"
          ></i>
          <span
            i18n-id="common.ui.vis.editors.advancedToggle.advancedLinkLabel"
            i18n-default-message="Computed Fields"
          >
            Computed Fields
          </span>
        </a>
      ...
      <div className="kuiSideBarFormRow ng-hide" id="jsonText">
        <div className="kuiSideBarFormRow__control">
          <textarea
            type="text"
            id="visEditorRawJson2"
            className="form-control ng-pristine ng-valid ng-empty ng-valid-json-input ng-touched"
            validate-json=""
            aria-invalid="false"
            ng-model="editorState.params.jsonInput"
          ></textarea>
        </div>
        <br></br>
      </div>
      ...
      <label className="kuiSideBarFormRow__label">Available Fields </label>
      <div className="kuiSideBarFormRow">
        <select multiple className="form-control" id="fieldsList"></select>
      </div>
      <br></br>
      <div className="kuiSideBarFormRow">
        <div className="col-md-12">
      ...
      <label className="kuiSideBarFormRow__label">View Fields </label>
      <div className="kuiSideBarFormRow">
        <div className="selected-left">
          <select multiple className="form-control" id="viewList"></select>
        </div>
      ...
      <br></br>
      <br></br>
      <label className="kuiSideBarFormRow__label">Export Fields </label>
      <div className="kuiSideBarFormRow">
        <div className="selected-left">
          <select multiple className="form-control" id="exportList"></select>
        </div>
