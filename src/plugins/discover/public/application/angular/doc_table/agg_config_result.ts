...
export function AggConfigResult(aggConfig, parent, value, key, filters) {
  this.key = key;
  this.value = value;
...
AggConfigResult.prototype.toString = function (contentType) {
  ...
  if(this.aggConfig.highlight){
	  return this.aggConfig.fieldFormatter(contentType)(this.value.value, this.aggConfig.params.field, this.value.hit, parsedUrl); 
	  }else{
